...
sfe <- alabaster.base::readObject("results/day1/01.1_sfe_visium/")
...
Now, we will calculate per-cell QC metrics using the `scrapper` package. The function `quickRnaQc.se` adds QC metrics, including mitochondrial gene percentage, to the `colData` of the `SpatialFeatureExperiment` object. Run the following code to compute these metrics:
...
p2 <- ggcells(sfe, aes(x=detected, y=subset.proportion.Mito*100)) + 
  geom_point() +
  geom_density_2d() +
  scale_x_log10() +
  geom_xsidehistogram() + 
...
sfe$qc_mito_prop <- sfe$subset.proportion.Mito > 0.1
...
```{r}
## Plot the metrics
p1 <- plotSpatialFeature(sfe, colGeometryName = "cellSeg", features = "subset.proportion.Mito") +
...
sfe$local_outliers <- as.logical(sfe$sum_outliers) |
  ...
  as.logical(sfe$subset.proportion.Mito_outliers)
```
...
```{r}
plotQCmetrics(sfe,
  metric = "subset.proportion.Mito", outliers = "subset.proportion.Mito_outliers",
...
```{r}
plotObsQC(sfe, plot_type = "violin", x_metric = "subset.proportion.Mito_z", annotate = "qc_mito_prop")
```
...
sfe$qc_mito_prop <- sfe$subset.proportion.Mito > 30
...
sfe$subsets_Mito_sum <- colSums(counts(sfe)[is.mito, ])

sfe <- tryCatch(
  findArtifacts(sfe,
    mito_percent = "subset.proportion.Mito",
...
-   `scrapper::quickRnaQc.se()` (anciently `scuttle::addPerCellQCMetrics`) provides essential per-bin QC metrics.

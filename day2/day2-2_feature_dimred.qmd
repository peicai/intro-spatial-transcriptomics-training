...
We will start by loading the normalized `SpatialFeatureExperiment` object from the previous exercise, which contains filtered cells, and genes with at least 1 count across all retained cells.

```{r}
# Reload the SpatialFeatureExperiment object if not in the R session already
sfe <- alabaster.base::readObject("results/day1/01.2_sfe_visium/")
sfe
```
...
sfe <- scrapper::chooseRnaHvgs.se(sfe, top=1000)

## All information is stored in rowData. Have a look a the new columns added
rowData(sfe)

## Sort by residual variance, i.e., likely the biological variance
rowData(sfe) |> as_tibble() |> dplyr::arrange(desc(residuals))
...
plot(rowData(sfe)$means, rowData(sfe)$variances, col=factor(rowData(sfe)$hvg), 
     xlab = "Mean expression (log-counts)", ylab = "Total variance (log-counts)", main = "Mean-Variance Trend")
curve(approxfun(rowData(sfe)$means, rowData(sfe)$fitted)(x), col="dodgerblue", add=TRUE)
```
...
# Top 6 HVGs
top_HVG <- row.names(sfe)[order(rowData(sfe)$residuals, decreasing = TRUE)][1:6]
top_HVG
...
hvg <- row.names(sfe)[rowData(sfe)$hvg]

# Display the range of biological variation for the selected HVGs
rowData(sfe)$residuals[rowData(sfe)$hvg] |> summary()
```
...
TO DO: run in on the list of random HVGs, otherwise all 20 top HVGs are also SVGs...
-->
...
- Select the top SVGs detected by `nnSVG`, what do you notice?
- Bonus: rerun `nnSVG` on a subset of randomly chosen HVGs
:::
...
```{r}
alabaster.sfe::saveObject(sfe, "results/day1/02.2_sfe")
```

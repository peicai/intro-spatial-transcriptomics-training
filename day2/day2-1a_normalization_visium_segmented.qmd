...
The object contains raw counts. We use `scrapper::normalizeRnaCounts.se()`, a faster replacement for the `scuttle::logNormCounts()` function to compute size factors and add a `logcounts` assay.
...
*NOTE*: Other more advanced normalization methods exist for scRNA-seq data, which aim at adjusting for compositional biases across cells, for example `scuttle::computePooledFactors()`, or at stabilizing the variance of such low-counts data, for example `Seurat:SCTranform()`
...
alabaster.sfe::saveObject(sfe, "results/day1/02.1_sfe")
```
